...
describe("list animation retrigger fix (#410)", () => {
  it("does not remount spans for existing list items when a new item appears", async () => {
    ...
    // The list is loose once a second group appears. This fork unwraps the
    // sole paragraph child of a loose `li`, so there is no `li > p` in the
    // DOM — the items still rebuild. What must not happen is the
    // already-visible text re-running its entry animation: those characters
    // carry --sd-duration:0ms.
