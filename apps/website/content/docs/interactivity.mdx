...
To hook the default copy button without replacing the control, pass callbacks through `controls.code.copy` (the same shape works for `mermaid`, `plantuml`, and `openscad`):
