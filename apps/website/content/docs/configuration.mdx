...
You can still use `download: true` (or omit it) to keep the default filenames: `file.<ext>` for code, `table.csv` / `table.md` for tables, `diagram.svg` / `diagram.png` / `diagram.mmd` for mermaid, and `diagram.svg` / `diagram.png` / `diagram.puml` for PlantUML.
